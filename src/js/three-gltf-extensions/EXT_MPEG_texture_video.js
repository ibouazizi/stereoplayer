...
export class GLTFMPEGTextureVideoExtension {
    ...
    async loadTexture( textureIndex ) {
        ...
        return parser.getDependency( 'accessor', extensionDef.accessor ).then( accessor => {
            ...
            return new Promise( async resolve => {
                ...
                const maxFrames = 5; // Increase buffer size to handle larger frames
                ...
                texture.renderer = this.renderer;
                ...
                // Add stereo metadata if present in extras
                if (extensionDef.extras) {
                    if (extensionDef.extras.group_id !== undefined) {
                        texture.userData.groupId = extensionDef.extras.group_id;
                        texture.userData.groupType = extensionDef.extras.group_type;
                        texture.userData.mask = extensionDef.extras.mask;
                    }
                }
                ...
                // Wait for OpenCV to be ready in main thread
                await window.opencvReady;
                
                // Notify worker that OpenCV is ready
                    ...
                    if (availableData >= this.frameSize) {
                        try {
                            ...
                            if (bytesRead === this.frameSize) {                                
                                ...
                                // Force renderer to update this texture
                                if (this.renderer) {
                                    this.renderer.initTexture(this);
                                }
                                
                            } else {
                                console.warn('[Texture] Incomplete frame data:', bytesRead, 'bytes of', this.frameSize);
                            }
                        } catch (error) {
                            console.error('[Texture] Error updating:', error);
                        }
                    }
                };
    ...
    afterRoot( result ) {
        ...
        // Create a map to store stereo texture groups
        const stereoGroups = new Map();

        // First pass: collect stereo textures
        result.scene.traverse(node => {
            if (node.material && node.material.map && node.material.map.update) {
                const texture = node.material.map;
                if (texture.userData.groupId !== undefined && texture.userData.groupType === 'stereo') {
                    if (!stereoGroups.has(texture.userData.groupId)) {
                        stereoGroups.set(texture.userData.groupId, {
                            left: null,
                            right: null,
                            materials: new Set()
                        });
                    }
                    const group = stereoGroups.get(texture.userData.groupId);
                    if (texture.userData.mask === 'left') {
                        group.left = texture;
                    } else if (texture.userData.mask === 'right') {
                        group.right = texture;
                    }
                    group.materials.add(node.material);
                }
            }
        });

        // Second pass: set up materials and textures
        result.scene.traverse(node => {
            if (node.material) {
                ...
                if (node.material.map && node.material.map.update) {
                    ...
                    if (texture.userData.groupId !== undefined && texture.userData.groupType === 'stereo') {
                        ...
                        // Store original and stereo textures in material
                        node.material.userData.originalMap = texture;
                        node.material.userData.stereoGroup = group;

                        // Add onBeforeRender to handle XR mode
                        node.material.onBeforeRender = function(renderer, scene, camera, geometry, object, group) {
                            if (renderer.xr.isPresenting) {
                                // In XR mode, use the appropriate texture based on the current eye
                                const currentCamera = renderer.xr.getCamera();
                                const isLeftEye = camera === currentCamera.cameras[0];
                                
                                if (this.userData.stereoGroup) {
                                    this.map = isLeftEye ? 
                                        this.userData.stereoGroup.left : 
                                        this.userData.stereoGroup.right;
                                }
                            } else {
                                // In non-XR mode, use the original texture
                                this.map = this.userData.originalMap;
                            }
                        };
                    }
                }
        ...
        // Store stereo groups in the scene for reference
        result.userData.stereoGroups = stereoGroups;
    }
}